...
input_files_dir = './data/case_11_pss_on_soft_soil/'
results_dir = "output_soft_soil_pss"
...
soil_formulation_1 = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=1850, POROSITY=0.0
)
constitutive_law_1 = LinearElasticSoil(YOUNG_MODULUS=150e6, POISSON_RATIO=0.2)
material_ballast = SoilMaterial(
    "ballast", soil_formulation_1, constitutive_law_1, SaturatedBelowPhreaticLevelLaw()
)

soil_formulation_embankment = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=1900, POROSITY=0.0
)
...
# fill in PSS parameters, assumed 10x stiffer than ballast and about 5x
# stiffer than very stiff sand
soil_formulation_pss = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=2000, POROSITY=0.0
)
constitutive_law_pss = LinearElasticSoil(
    YOUNG_MODULUS=1.5e9, POISSON_RATIO=poisson_ratio_undrained
)
material_pss = SoilMaterial(
    "pss", soil_formulation_pss, constitutive_law_pss, SaturatedBelowPhreaticLevelLaw()
)

soil_formulation_clay = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=1500, POROSITY=0.0
)
constitutive_law_clay = LinearElasticSoil(
    YOUNG_MODULUS=50e6, POISSON_RATIO=poisson_ratio_undrained
)
material_clay = SoilMaterial(
    "clay",
    soil_formulation_clay,
    constitutive_law_clay,
    SaturatedBelowPhreaticLevelLaw(),
)

soil_formulation_peat = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=1400, POROSITY=0.0
)
constitutive_law_peat = LinearElasticSoil(
    YOUNG_MODULUS=100e6, POISSON_RATIO=poisson_ratio_undrained
)
material_peat = SoilMaterial(
    "peat",
    soil_formulation_peat,
    constitutive_law_peat,
    SaturatedBelowPhreaticLevelLaw(),
)
...
constitutive_law_sand = LinearElasticSoil(
    YOUNG_MODULUS=350e6, POISSON_RATIO=poisson_ratio_undrained
)
material_sand = SoilMaterial(
    "sand",
    soil_formulation_sand,
...
soil_formulation_styrofoam = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=32, POROSITY=0.0
)
constitutive_law_styrofoam = LinearElasticSoil(
    YOUNG_MODULUS=5e6, POISSON_RATIO=poisson_ratio_undrained
)
material_styrofoam = SoilMaterial(
    "styrofoam",
    soil_formulation_styrofoam,
    constitutive_law_styrofoam,
    SaturatedBelowPhreaticLevelLaw(),
)

young_modulus_water_in_ditch = 30e4  # small value to avoid numerical issues
soil_formulation_water_in_ditch = OnePhaseSoil(
    ndim, IS_DRAINED=True, DENSITY_SOLID=1000, POROSITY=0.0
)
constitutive_law_water_in_ditch = LinearElasticSoil(
    YOUNG_MODULUS=young_modulus_water_in_ditch, POISSON_RATIO=poisson_ratio_undrained
)
material_water_in_ditch = SoilMaterial(
    "water_in_ditch",
    soil_formulation_water_in_ditch,
    constitutive_law_water_in_ditch,
    SaturatedBelowPhreaticLevelLaw(),
)
...
deep_wall_part_1_coordinates = [
    (x_coord_deep_wall, surface_level, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, surface_level, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, layer_1_boundary_depth, 0.0),
    (x_coord_deep_wall, layer_1_boundary_depth, 0.0),
]
deep_wall_part_2_coordinates = [
    (x_coord_deep_wall, layer_1_boundary_depth, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, layer_1_boundary_depth, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, layer_2_boundary_depth, 0.0),
    (x_coord_deep_wall, layer_2_boundary_depth, 0.0),
]
deep_wall_part_3_coordinates = [
    (x_coord_deep_wall, layer_2_boundary_depth, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, layer_2_boundary_depth, 0.0),
    (
        x_coord_deep_wall + thickness_deep_wall,
        layer_2_boundary_depth - length_in_sand,
        0.0,
    ),
    (x_coord_deep_wall, layer_2_boundary_depth - length_in_sand, 0.0),
]

# coordinates for the ditch with a 1:1 slope
depth_ditch = 1.5
width_bottom_ditch = 2
x_coordinate_end_ditch = (
    x_coord_deep_wall + thickness_deep_wall + depth_ditch * 2 + width_bottom_ditch
)
ditch_coordinates = [
    (x_coord_deep_wall + thickness_deep_wall, surface_level, 0.0),
    (
        x_coord_deep_wall + thickness_deep_wall + depth_ditch,
        surface_level - depth_ditch,
        0.0,
    ),
    (
        x_coord_deep_wall + thickness_deep_wall + depth_ditch + width_bottom_ditch,
        surface_level - depth_ditch,
        0.0,
    ),
...
soil_1_coordinates_part_1 = [
    (0.0, surface_level - thickness_foundation_bottom - thickness_foundation_top, 0.0),
    (7.5, surface_level - thickness_foundation_bottom - thickness_foundation_top, 0.0),
    (7.5, surface_level - thickness_foundation_top, 0.0),
    (7.5, surface_level, 0.0),
    (x_coord_deep_wall, surface_level, 0.0),
    (x_coord_deep_wall, layer_1_boundary_depth, 0.0),
    (0.0, layer_1_boundary_depth, 0.0),
]
soil_1_coordinates_part_2 = ditch_coordinates + [
    (max_x_coordinate, surface_level, 0.0),
    (max_x_coordinate, layer_1_boundary_depth, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, layer_1_boundary_depth, 0.0),
]

soil_2_coordinates_part_1 = [
    (0.0, layer_1_boundary_depth, 0.0),
    (x_coord_deep_wall, layer_1_boundary_depth, 0.0),
    (x_coord_deep_wall, layer_2_boundary_depth, 0.0),
    (0.0, layer_2_boundary_depth, 0.0),
]
soil_2_coordinates_part_2 = [
    (x_coord_deep_wall + thickness_deep_wall, layer_1_boundary_depth, 0.0),
    (max_x_coordinate, layer_1_boundary_depth, 0.0),
    (max_x_coordinate, layer_2_boundary_depth, 0.0),
    (x_coord_deep_wall + thickness_deep_wall, layer_2_boundary_depth, 0.0),
]
...
model.add_soil_layer_by_coordinates(
    embankment_coordinates, material_embankment, "embankment"
)

model.add_soil_layer_by_coordinates(
    foundation_coordinates_top, material_pss, "foundation_top"
)
model.add_soil_layer_by_coordinates(
...
model.add_soil_layer_by_coordinates(
    soil_1_coordinates_part_1, material_clay, "soil_layer_1_part_1"
)
model.add_soil_layer_by_coordinates(
    soil_1_coordinates_part_2, material_clay, "soil_layer_1_part_2"
)

model.add_soil_layer_by_coordinates(
    soil_2_coordinates_part_1, material_peat, "soil_layer_2_part_1"
)
model.add_soil_layer_by_coordinates(
    soil_2_coordinates_part_2, material_peat, "soil_layer_2_part_2"
)
...
model.add_soil_layer_by_coordinates(
    deep_wall_part_1_coordinates, material_clay, "deep_wall_part_1"
)
model.add_soil_layer_by_coordinates(
    deep_wall_part_2_coordinates, material_peat, "deep_wall_part_2"
)
model.add_soil_layer_by_coordinates(
    deep_wall_part_3_coordinates, material_sand, "deep_wall_part_3"
)
...
soil_equivalent_parameters = ElasticSpringDamper(
    NODAL_DISPLACEMENT_STIFFNESS=[0, 71e6, 0],
    NODAL_ROTATIONAL_STIFFNESS=[0, 0, 0],
...
uvec_parameters = {
    ...
    "cart_damping": 64e3,
    # distances of the bogies from the centre of the cart [m]
    "bogie_distances": [-9.95, 9.95],
    # inertia of the bogie [kgm2]
    ...
    # axis on which gravity works [x =0, y = 1, z = 2]
    "gravity_axis": 1,
    # Hertzian contact coefficient between the wheel and the
    # rail [N/m]
...
uvec_load = UvecLoad(
    ...
    velocity=0.0,
    origin=[0.7, 2.6 + rail_pad_thickness, -43],
    wheel_configuration=wheel_configuration,
    uvec_model=uvec,
...
model.add_boundary_condition_on_plane(
    [(0, bottom_coordinate, 0), (0, bottom_coordinate, 1), (1, bottom_coordinate, 0)],
    absorbing_boundaries_parameters_bottom,
    "bottom_abs",
)

model.add_boundary_condition_on_plane(
    [(0, 0, 0), (0, 1, 0), (0, 0, 1)], roller_displacement_parameters, "sides_roller"
)

model.add_boundary_condition_on_plane(
    [(0, 0, 0), (1, 0, 0), (0, 1, 0)], absorbing_boundaries_parameters, "abs"
)
model.add_boundary_condition_on_plane(
    [
        (0, 0, model.extrusion_length),
        (1, 0, model.extrusion_length),
        (0, 1, model.extrusion_length),
    ],
    absorbing_boundaries_parameters,
    "abs",
)
model.add_boundary_condition_on_plane(
    [(max_x_coordinate, 0, 0), (max_x_coordinate, 1, 0), (max_x_coordinate, 0, 1)],
    absorbing_boundaries_parameters,
    "abs",
)
...
json_output_parameters = JsonOutputParameters(
    delta_time - 1e-10, [NodalOutput.VELOCITY], []
)
model.add_output_settings_by_coordinates(
    [
        (x_coordinate_end_ditch, surface_level, 45.0),
        (25, surface_level, 45.0),
        (max_x_coordinate, surface_level, 45),
    ],
    json_output_parameters,
    "case_11_pss_on_soft_soil"
)
